...
class MongoDBmanagement:
    ...
    def __init__(self):
        ...
        try:
            self.username = 'atufashireen'
            self.password = '7rrww1b9aQcv9mSF'
            self.url = f"mongodb+srv://{self.username}:{self.password}@analytics.lmbi7.mongodb.net/myFirstDatabase?retryWrites=true&w=majority"
            self.mongo_client = pymongo.MongoClient(self.url)
        except Exception as e:
            raise Exception("At __init__:",e)
    # ask if i should keep the connection till the obj is referenced of call it everytime
    def __del__(self):
    ...
    def isCollectionPresent(self, collection_name, db_name):
        ...
        try:
            ...
            print(f'ldkgdofigjdfoi-{database_status}')
            if database_status:
